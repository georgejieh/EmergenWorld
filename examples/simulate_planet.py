...
def compare_planet_properties():
    ...
    for i, (planet, title) in enumerate(zip(planets, titles)):
        ...
        planet.visualize_solar_radiation(
            ax=axes[i, 1], title=f"Solar Radiation - {title}")
...
def create_fantasy_world():
    ...
    print(f"Planet radius: {planet.planet_radius_km:.1f} km")
    print(f"Grid cell size: {planet.km_per_cell:.1f} km")
    print(f"Day length: {planet.day_length_hours} hours")
    print(f"Year length: {planet.year_length_days} days")

    # Demonstrate day cycle
    demonstrate_day_cycle(planet, steps=16)

    # Demonstrate seasonal cycle
    demonstrate_seasonal_cycle(planet, steps=12)
