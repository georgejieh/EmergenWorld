...
class TerrainGenerator:
    ...
    def __init__(self, size: int = 1024, octaves: int = 6, 
        ...
        self.area_per_cell_sqmiles = self.area_per_cell_sqkm * 0.386102  # Convert to square miles
        ...
        print(f"Each grid cell represents {self.km_per_cell:.2f} km ({self.area_per_cell_sqmiles:.2f} sq miles)")
    ...
    def add_rivers(self, 
        ...
        for idx in source_indices:
            ...
            for _ in range(self.size * 2):  # Maximum river length
                    ...
                    for dx in [-1, 0, 1]:
                        ...
                        if 0 <= ny < self.size and 0 <= nx < self.size:
                            ...
                            if len(river_path) > 1:
                                ...
                                if dy * flow_dy + dx * flow_dx > 0:  # Dot product > 0 means similar direction
                                    direction_weight = 1.5
                                elif dy * flow_dy + dx * flow_dx < 0:  # Opposite direction
                            ...
                            # Calculate the weighted score - lower is better (we want to go downhill)
                ...
                if next_x <= 1 or next_x >= self.size - 2 or next_y <= 1 or next_y >= self.size - 2:
            ...
            if len(river_path) >= min_length:
                ...
                for py, px in river_path:
                    ...
                    if river_width > 1:
                        for wy in range(max(0, py-river_width//2), min(self.size, py+(river_width+1)//2)):
                            for wx in range(max(0, px-river_width//2), min(self.size, px+(river_width+1)//2)):
                                if (wy - py)**2 + (wx - px)**2 <= (river_width//2)**2:  # Circular brush
                                    river_mask[wy, wx] = 1
    ...
    def add_lakes(self, 
        ...
        print(f"Generating inland lakes...")
    ...
    def generate_complete_water_system(self, 
        ...
        lake_mask = self.add_lakes(count=lake_count, min_size=10, max_size=100, ocean_mask=ocean_mask)
...
if __name__ == "__main__":
    # Example usage for fantasy world generation
    print("Generating a fantasy world at 0.83% of Earth's size...")
    generator = TerrainGenerator(size=256, seed=42, earth_scale=0.0083)
    
    # Generate base terrain
    heightmap = generator.generate_heightmap(scale=150.0)
    generator.visualize(title="Base Heightmap", show_grid=True)
    ...
    generator.add_mountains(mountain_scale=1.2, peak_threshold=0.6, epic_factor=2.5)
    ...
    water_mask, water_systems = generator.generate_complete_water_system(
        ocean_coverage=0.65,  # Slightly less ocean to make room for lakes and rivers
        river_count=25,
        lake_count=15
    )
    
    # Visualize the complete water system
    generator.visualize_water_system(water_systems, title="Fantasy World Water Systems")
    
    # Visualize the final world
    generator.visualize(water_mask=water_mask, title="Complete Fantasy World", show_grid=True)
    ...
    print(f"Agents per sq mile: 10")
    print(f"Estimated total agent population: {int(total_land_area_sqmiles * 10):,}")
