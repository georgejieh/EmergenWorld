...
class PlanetarySystem:
    ...
    def __init__(
        ...
        self.planet_radius_km = 6371.0 * np.sqrt(earth_scale)  # Earth radius × scale factor
        ...
        print(f"Planet radius: {self.planet_radius_km:.1f} km, Cell size: {self.km_per_cell:.1f} km")
    ...
    def _initialize_coordinates(self) -> None:
        ...
        for y in range(self.world_size):
            lat = 90 - (y / (self.world_size - 1) * 180)
            self.latitudes[y, :] = lat
            
        # Calculate longitudes (from -180° at the left to +180° at the right)
        for x in range(self.world_size):
            lon = (x / (self.world_size - 1) * 360) - 180
            self.longitudes[:, x] = lon
    ...
    def _setup_celestial_objects(self) -> None:
        ...
        self.observer.pressure =.0  # Ignore atmospheric refraction
    ...
    def _update_sun_position(self) -> None:
        ...
        # Calculate the day of year as a fraction
        day_fraction = self.current_day / self.year_length_days
    ...
    def _update_day_night_cycle(self) -> None:
        ...
        for y in range(self.world_size):
            for x in range(self.world_size):
                lat = self.latitudes[y, x]
                lon = self.longitudes[y, x]
    ...
    def visualize_day_length(self, latitudes=None):
        ...
        for lat in latitudes:
            days = np.arange(0, self.year_length_days, self.year_length_days/50)  # Sample points
            day_lengths = []
            
            for day in days:
                self.current_day = int(day)
                day_lengths.append(self.get_day_length(lat))
                
            plt.plot(days, day_lengths, label=f"{lat}° latitude")
            
        plt.axhline(y=self.day_length_hours/2, color='black', linestyle='--', alpha=0.3, 
                   label="12 hours")
        
        # Mark seasons (for northern hemisphere)
        season_days = [0, self.year_length_days/4, self.year_length_days/2, 3*self.year_length_days/4]
        season_names = ["Winter", "Spring", "Summer", "Fall"]
        
        for day, name in zip(season_days, season_names):
            plt.axvline(x=day, color='gray', linestyle=':', alpha=0.5)
            plt.text(day, self.day_length_hours * 0.1, name, 
                    rotation=90, va='bottom', ha='center', alpha=0.7)
...
if __name__ == "__main__":
    # Example usage
    print("Simulating planetary system for fantasy world...")
    
    # Create a fantasy planetary system
    planet = PlanetarySystem(
        world_size=256,
        axial_tilt_degrees=28.0,  # More extreme than Earth
    ...
    # Update the system to a specific time (mid-year, noon)
    planet.current_day = int(planet.year_length_days / 2)  # Mid-year (summer in northern hemisphere)
    ...
    # Display current date and season
    print(f"Current date: {planet.get_formatted_date()}")
    print(f"Current season: {planet.get_season()}")
    
    # Visualize day/night cycle
    plt.figure(figsize=(10, 5))
    planet.visualize_day_night()
    plt.tight_layout()
    plt.show()
    
    # Visualize solar radiation
    plt.figure(figsize=(10, 5))
    planet.visualize_solar_radiation()
    plt.tight_layout()
    plt.show()
    
    # Visualize day length throughout the year
    planet.visualize_day_length()
    
    # Calculate day length at different latitudes
    print("\nDay lengths at different latitudes:")
    for lat in [0, 28, 45, 66.5, 80]:
        hours = planet.get_day_length(lat)
        print(f"  {lat}° latitude: {hours:.1f} hours of daylight")
    
    # Calculate seasonal temperature factors
    print("\nSeasonal temperature modifiers:")
    for lat in [0, 28, 45, 66.5, 80]:
        temp_mod = planet.get_seasonal_temperature_factor(lat)
        print(f"  {lat}° latitude: {temp_mod:.1f}°C")